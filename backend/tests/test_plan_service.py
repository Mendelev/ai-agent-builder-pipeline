...
def test_calculate_complexity():
    ...
    req_map = {
        "REQ-001": Requirement(key="REQ-001", priority="critical", acceptance_criteria=["AC1", "AC2", "AC3"]),
        "REQ-002": Requirement(key="REQ-002", priority="medium", acceptance_criteria=["AC1"]),
    }
...
def test_estimate_duration():
    ...
    group = ["REQ-001"]
    req_map = {"REQ-001": Requirement(key="REQ-001", priority="high", dependencies=["DEP-001", "DEP-002"])}
...
def test_generate_plan_with_high_priority_uncovered(db_session: Session, sample_project: Project):
    ...
    reqs = [
        ...
        Requirement(
            project_id=sample_project.id, key="NORMAL-001", title="Normal Priority", priority="medium", is_coherent=True
        ),
    ]
...
def test_plan_versioning(db_session: Session, sample_project: Project):
    ...
    # Create requirement
    req = Requirement(project_id=sample_project.id, key="REQ-001", title="Test", priority="medium", is_coherent=True)
    db_session.add(req)
    db_session.commit()
