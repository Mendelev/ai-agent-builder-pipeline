...
def test_transition_state_valid(db_session: Session, sample_project: Project):
    ...
    history = db_session.query(StateHistory).filter(
        StateHistory.project_id == sample_project.id
    ).first()
...
def test_transition_state_invalid(db_session: Session, sample_project: Project):
    ...
    success, error = OrchestrationService.transition_state(
        db_session,
        sample_project.id,
        ProjectState.DONE,  # Invalid transition
        reason="Invalid jump"
    )
...
def test_check_and_get_dedup_new(db_session: Session, sample_project: Project):
    ...
    dedup = db_session.query(DedupKey).filter(
        DedupKey.project_id == sample_project.id,
        DedupKey.agent_type == AgentType.REFINE
    ).first()
...
def test_record_agent_execution(db_session: Session, sample_project: Project):
    ...
    audit = db_session.query(AuditLog).filter(
        AuditLog.project_id == sample_project.id,
        AuditLog.agent_type == AgentType.PLAN
    ).first()
    ...
    event = db_session.query(DomainEvent).filter(
        DomainEvent.aggregate_id == sample_project.id,
        DomainEvent.event_name == "agent_plan_completed"
    ).first()
...
def test_get_audit_logs_pagination(db_session: Session, sample_project: Project):
    ...
    # Get first page
    result = OrchestrationService.get_audit_logs(
        db_session,
        sample_project.id,
        page=1,
        page_size=10
...
def test_retry_agent_allowed(db_session: Session, sample_project: Project):
    ...
    audit = db_session.query(AuditLog).filter(
        AuditLog.project_id == sample_project.id,
        AuditLog.event_type == EventType.RETRY_ATTEMPTED
    ).first()
