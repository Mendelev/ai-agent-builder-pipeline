...
from app.models.orchestration import AgentType, AuditLog, EventType
...
def test_get_audit_logs_paginated(client: TestClient, sample_project: Project, db_session: Session):
    ...
    for i in range(25):
        audit = AuditLog(
            project_id=sample_project.id, event_type=EventType.SYSTEM_EVENT, action=f"Test action {i}", success=True
        )
        db_session.add(audit)
    db_session.commit()

    # Get first page
    response = client.get(f"/api/v1/projects/{sample_project.id}/audit?page=1&page_size=10")
    ...
    # Get second page
    response = client.get(f"/api/v1/projects/{sample_project.id}/audit?page=2&page_size=10")
...
def test_retry_agent_execution(client: TestClient, sample_project: Project):
    ...
    response = client.post(
        ...
        json={"agent": "REFINE", "force": False, "metadata": {"test": "data"}},
    )
...
def test_retry_invalid_agent(client: TestClient, sample_project: Project):
    ...
    response = client.post(
        ...
        json={"agent": "REQUIREMENTS", "force": False},  # Valid in JSON, but URL has invalid agent
    )
...
def test_get_audit_logs_invalid_event_type(client: TestClient, sample_project: Project):
    ...
    response = client.get(f"/api/v1/projects/{sample_project.id}/audit?event_type=INVALID_TYPE")
...
def test_retry_agent_invalid_project(client: TestClient):
    ...
    non_existent_id = uuid.uuid4()
    response = client.post(f"/api/v1/projects/{non_existent_id}/retry/refine", json={"agent": "REFINE", "force": False})
