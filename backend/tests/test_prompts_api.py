...
from app.models import Plan, PlanPhase, Project, PromptBundle, Requirement


def test_generate_prompts_no_plan(client: TestClient, sample_project: Project):
    """Test prompt generation fails without plan."""
    response = client.post(f"/api/v1/projects/{sample_project.id}/prompts/generate", json={"include_code": False})

    assert response.status_code == 400
    assert "No plan found" in response.json()["detail"]


def test_generate_prompts_with_plan(client: TestClient, sample_project: Project, db_session: Session):
    """Test successful prompt generation."""
    # Create plan with phases
    plan = Plan(project_id=sample_project.id, version=1, source="requirements", total_duration_days=20.0)
    db_session.add(plan)
    db_session.flush()
    ...
    with patch("app.workers.tasks.prompts.generate_prompts.apply_async") as mock_task:
        ...
        response = client.post(
            ...
            json={"include_code": True, "plan_id": str(plan.id)},
        )
...
def test_get_latest_prompts(client: TestClient, sample_project: Project, db_session: Session):
    ...
    prompt1 = PromptItem(
        bundle_id=bundle.id, phase_id="PH-01", sequence=1, title="Phase 1", content_md="# Phase 1 Prompt"
    )
    prompt2 = PromptItem(
        bundle_id=bundle.id, phase_id="PH-02", sequence=2, title="Phase 2", content_md="# Phase 2 Prompt"
    )
...
def test_download_bundle_zip(client: TestClient, sample_project: Project, db_session: Session):
    ...
    # Download ZIP
    response = client.get(f"/api/v1/projects/{sample_project.id}/prompts/bundles/{bundle.id}/download")
...
def test_download_nonexistent_bundle(client: TestClient, sample_project: Project):
    ...
    response = client.get(f"/api/v1/projects/{sample_project.id}/prompts/bundles/{fake_id}/download")
...
def test_generate_prompts_invalid_project(client: TestClient):
    ...
    response = client.post(f"/api/v1/projects/{non_existent_id}/prompts", json={"formats": ["json"]})
    assert response.status_code == 404
...
def test_generate_prompts_exception_handling(client: TestClient, sample_project: Project):
    ...
    response = client.post(f"/api/v1/projects/{sample_project.id}/prompts", json={"formats": ["invalid_format"]})
...
def test_download_bundle_invalid_project(client: TestClient):
    ...
    response = client.get(f"/api/v1/projects/{non_existent_project_id}/prompts/bundles/{bundle_id}/download")
    assert response.status_code == 404
