...
def test_sanitize_nested_content():
    ...
    data = {
        "config": {"api_key": "secret123", "database": "postgresql://user:password@host/db"},
...
def test_generate_context(db_session: Session, sample_project: Project):
    ...
    plan = Plan(
        ...
        risk_score=0.5,
        constraints={"deadline_days": 90, "team_size": 5, "nfrs": ["Performance", "Security"]},
    )
    db_session.add(plan)
    db_session.flush()  # This assigns an ID to plan

    phase = PlanPhase(
        plan_id=plan.id, phase_id="PH-01", sequence=1, title="Phase 1", objective="Test", estimated_days=10.0
    )
    db_session.add(phase)

    requirements = [
        Requirement(
            project_id=sample_project.id, key="REQ-001", title="Critical Feature", priority="critical", is_coherent=True
        ),
        Requirement(
            project_id=sample_project.id,
            key="REQ-002",
    ...
    # Generate context
    context = PromptService._generate_context(sample_project, plan, requirements, include_code=True)
...
def test_generate_phase_prompt(db_session: Session, sample_project: Project):
    ...
    prompt = PromptService._generate_phase_prompt(phase, requirements, sample_project, include_code=True)
...
def test_common_errors_generation():
    ...
    security_phase = PlanPhase(
        phase_id="PH-01", title="Security Implementation", objective="Implement security features"
    )
...
def test_create_bundle_zip(db_session: Session, sample_project: Project):
    ...
    bundle = PromptBundle(
        project_id=sample_project.id, plan_id=uuid.uuid4(), version=1, context_md="# Context", total_prompts=1
    )
    ...
    prompt = PromptItem(
        bundle_id=bundle.id, phase_id="PH-01", sequence=1, title="Phase 1", content_md="# Phase 1 Content"
    )
