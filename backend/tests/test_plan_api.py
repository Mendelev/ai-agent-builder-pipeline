...
from app.models import Plan, Project, Requirement
...
def test_generate_plan_no_requirements(client: TestClient, sample_project: Project):
    ...
    response = client.post(
        ...
        json={"source": "requirements", "use_code": False, "include_checklist": False},
    )
    ...
    with patch("app.workers.tasks.plan.generate_plan.apply_async") as mock_task:
        ...
        response = client.post(
            ...
            json={
                ...
                "constraints": {"deadline_days": 90, "team_size": 5},
            },
        )
...
def test_generate_plan_with_checklist(client: TestClient, sample_project: Project, db_session: Session):
    ...
    request = PlanGenerateRequest(source="requirements", include_checklist=True)
...
def test_get_plan_by_id(client: TestClient, sample_project: Project, db_session: Session):
    ...
    # Create a plan
    plan = Plan(project_id=sample_project.id, version=1, source="requirements")
    db_session.add(plan)
    db_session.commit()
...
def test_generate_plan_invalid_project(client: TestClient):
    ...
    response = client.post(f"/api/v1/projects/{non_existent_id}/plan", json={"source": "requirements"})
    assert response.status_code == 404
...
def test_generate_plan_exception_handling(client: TestClient, sample_project: Project):
    ...
    response = client.post(f"/api/v1/projects/{sample_project.id}/plan", json={"source": "invalid_source"})
