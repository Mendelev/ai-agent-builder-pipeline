...
from app.models.orchestration import AuditLog, DedupKey, DomainEvent
...
@celery_app.task(bind=True, base=MaintenanceTask, name="maintenance.cleanup_workdirs", max_retries=1)
def cleanup_workdirs(self, retention_days: int = 7):
    ...
    try:
        ...
        result = {"cleaned_count": cleaned_count, "duration_seconds": round(duration, 2)}
...
@celery_app.task(bind=True, base=MaintenanceTask, name="maintenance.cleanup_expired_dedup", max_retries=1)
def cleanup_expired_dedup(self):
    ...
    try:
        ...
        expired = self.db.query(DedupKey).filter(DedupKey.expires_at < datetime.now(UTC)).all()
        ...
        result = {"deleted_count": deleted_count, "duration_seconds": round(duration, 2)}
...
@celery_app.task(bind=True, base=MaintenanceTask, name="maintenance.process_domain_events", max_retries=1)
def process_domain_events(self, batch_size: int = 100):
    ...
    try:
        ...
        events = self.db.query(DomainEvent).filter(DomainEvent.processed_at.is_(None)).limit(batch_size).all()
        ...
        result = {"processed_count": processed_count, "duration_seconds": round(duration, 2)}
...
@celery_app.task(bind=True, base=MaintenanceTask, name="maintenance.retry_failed_tasks", max_retries=1)
def retry_failed_tasks(self, max_retries: int = 3):
    ...
    try:
        ...
        failed_logs = (
            ...
            .filter(AuditLog.created_at > cutoff, AuditLog.event_type == "AGENT_FAILED", not AuditLog.success)
            .all()
        )
        ...
        result = {"retried_count": retried_count, "duration_seconds": round(duration, 2)}
...
@celery_app.task(bind=True, base=MaintenanceTask, name="maintenance.enforce_retention", max_retries=1)
def enforce_retention(self, retention_days: int = 90):
    ...
    try:
        ...
        result = {"deleted_count": deleted, "retention_days": retention_days, "duration_seconds": round(duration, 2)}
