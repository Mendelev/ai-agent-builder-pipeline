...
from app.schemas.plan import PlanGenerateRequest, PlanResponse, PlanSummary, TaskPendingResponse
...
@router.post("", response_model=Union[PlanSummary, TaskPendingResponse])
async def generate_plan(project_id: UUID, request: PlanGenerateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        task = generate_plan_task.apply_async(args=[str(project_id), request.model_dump()], queue="default")
        ...
        try:
            ...
            if result["status"] == "completed":
                ...
                if plan:
                    ...
                    raise HTTPException(status_code=500, detail="Plan was generated but could not be retrieved")
            else:
...
async def get_latest_plan(project_id: UUID, db: Session = Depends(get_db)):
...
async def get_plan_by_id(project_id: UUID, plan_id: UUID, db: Session = Depends(get_db)):
