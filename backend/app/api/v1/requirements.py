...
from app.schemas.requirement import FinalizeRequest, RefineRequest, RequirementBulkCreate, RequirementResponse
...
async def create_requirements(
    project_id: UUID, payload: RequirementBulkCreate, request: Request, db: Session = Depends(get_db)
):
    ...
    if content_length and int(content_length) > settings.MAX_PAYLOAD_SIZE:
        raise HTTPException(
            status_code=413, detail=f"Payload too large. Maximum size is {settings.MAX_PAYLOAD_SIZE / (1024*1024)}MB"
        )

    try:
        requirements = RequirementService.create_bulk(db, project_id, payload.requirements)
...
async def list_requirements(project_id: UUID, db: Session = Depends(get_db)):
...
@router.post("/refine")
async def refine_requirements_endpoint(project_id: UUID, request: RefineRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Queue Celery task
        task = refine_requirements.apply_async(args=[str(project_id)], queue="default")
...
async def finalize_requirements_endpoint(project_id: UUID, request: FinalizeRequest, db: Session = Depends(get_db)):
    """Mark project as REQS_READY"""
    try:
        success = RequirementService.finalize_requirements(db, project_id, request.force)
...
async def export_requirements(
    project_id: UUID, format: str = Query("json", pattern="^(json|md)$"), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if format == "json":
            ...
            return JSONResponse(
                content=content, headers={"Content-Disposition": f"attachment; filename=requirements_{project_id}.json"}
            )
        else:  # markdown
            ...
            return PlainTextResponse(
                ...
                headers={"Content-Disposition": f"attachment; filename=requirements_{project_id}.md"},
            )
