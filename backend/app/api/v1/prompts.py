...
async def generate_prompts(
    ...
    try:
        ...
        task = generate_prompts_task.apply_async(
            args=[
                str(project_id),
                str(request.plan_id) if request.plan_id else str(plan.id),
...
async def get_latest_prompts(
    ...
    try:
        ...
        if not bundle:
            raise HTTPException(
                status_code=404,
                detail=f"No prompt bundle found for project {project_id}"
            )
...
async def download_prompt_bundle(
    project_id: UUID,
    bundle_id: UUID,
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if not bundle:
            raise HTTPException(
                status_code=404,
                detail=f"Bundle {bundle_id} not found for project {project_id}"
            )
