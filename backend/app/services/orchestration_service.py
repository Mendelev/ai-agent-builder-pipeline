...
from app.models.orchestration import AgentType, AuditLog, DedupKey, DomainEvent, EventType, ProjectState, StateHistory
...
class OrchestrationService:
    ...
    def transition_state(
        ...
        audit = AuditLog(
            ...
            details={"from_state": current_state.value, "to_state": to_state.value, "reason": reason},
    ...
    def get_project_status(db: Session, project_id: UUID) -> Dict[str, Any]:
        ...
        state_history = (
            ...
            .filter(StateHistory.project_id == project_id)
            .order_by(desc(StateHistory.created_at))
            .limit(5)
            .all()
        )
    ...
    @staticmethod
    def get_audit_logs(
        db: Session, project_id: UUID, page: int = 1, page_size: int = 50, event_type: Optional[EventType] = None
    ) -> Dict[str, Any]:
    ...
    def retry_agent(
        ...
        if not force:
            dedup = OrchestrationService.check_and_get_dedup(db, redis_client, project_id, agent_type, metadata or {})
            if dedup and dedup.result:
                return {"task_id": dedup.task_id, "status": "cached", "result": dedup.result}
        ...
        if task:
            return {"task_id": task.id, "status": "queued", "agent": agent_type.value}
