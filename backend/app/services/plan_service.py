...
from app.models import Plan, PlanPhase, PlanStatus, Project, Requirement
...
class PlanService:
    @staticmethod
    def generate_plan(db: Session, project_id: UUID, options: PlanGenerateRequest) -> Plan:
        ...
        last_plan = db.query(Plan).filter(Plan.project_id == project_id).order_by(desc(Plan.version)).first()
        ...
        # Generate phases
        phases = PlanService._generate_phases(requirements, options, project.context)
        ...
        total_risk = sum(risk_weights.get(p["risk_level"], 0.5) * p["estimated_days"] for p in phases)
    ...
    def _generate_phases(
        requirements: List[Requirement], options: PlanGenerateRequest, context: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        high_priority_reqs = [req for req in requirements if req.priority in ["high", "critical"]]
        ...
        phase_groups = PlanService._group_into_phases(execution_order, req_map, options.constraints)
    ...
    def _group_into_phases(
        execution_order: List[str], req_map: Dict[str, Requirement], constraints: Optional[Dict[str, Any]]
    ) -> List[List[str]]:
        """Group requirements into logical phases"""
    ...
    @staticmethod
    def _estimate_duration(group: List[str], req_map: Dict[str, Requirement], complexity: float) -> float:
        """Estimate phase duration in days"""
        # Base duration from complexity
        base_days = complexity * 2

        # Adjust for dependencies
        max_deps = max(len(req_map[k].dependencies or []) for k in group)
    ...
    @staticmethod
    def _estimate_resources(group: List[str], req_map: Dict[str, Requirement], complexity: float) -> Dict[str, int]:
        """Estimate required resources"""
        resources = {"developers": max(1, min(int(complexity / 3), 5)), "qa": max(1, int(len(group) / 3))}
    ...
    def get_latest_plan(db: Session, project_id: UUID) -> Optional[Plan]:
        ...
        return db.query(Plan).filter(Plan.project_id == project_id).order_by(desc(Plan.version)).first()

    @staticmethod
    def get_plan_by_id(db: Session, project_id: UUID, plan_id: UUID) -> Optional[Plan]:
        """Get specific plan by ID"""
        return db.query(Plan).filter(Plan.id == plan_id, Plan.project_id == project_id).first()
