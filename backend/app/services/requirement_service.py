...
from app.models import Project, ProjectState, Requirement, RequirementIteration
...
class RequirementService:
    ...
    def create_bulk(db: Session, project_id: UUID, requirements_data: List[RequirementCreate]) -> List[Requirement]:
        ...
        existing_reqs = db.query(Requirement).filter(Requirement.project_id == project_id).all()

        for req in existing_reqs:
            last_iteration = (
                db.query(RequirementIteration)
            ...
            if req_data.key in iteration_version_map:
                ...
                if changes:
                    ...
                    iteration = RequirementIteration(
                        requirement_id=existing.id, version=last_version + 1, changes=changes, created_by="system"
                    )
                    db.add(iteration)
                ...
                new_req = Requirement(project_id=project_id, **req_data.model_dump())
                db.add(new_req)
                db.flush()  # Get the ID

                # Create initial iteration
                iteration = RequirementIteration(
                    requirement_id=new_req.id, version=1, changes={"action": "created"}, created_by="system"
                )
                db.add(iteration)
    ...
    def finalize_requirements(db: Session, project_id: UUID, force: bool = False) -> bool:
        ...
        if not force:
            ...
            if missing_criteria:
                raise ValueError(
                    f"High/critical requirements missing acceptance criteria: {', '.join(missing_criteria)}"
                )
