...
from app.models import Plan, PlanPhase, Project, PromptBundle, PromptItem, Requirement
from app.schemas.prompt import PromptGenerateRequest
from app.utils.security import generate_placeholder_config, sanitize_content
...
class PromptService:
    @staticmethod
    def generate_prompts(db: Session, project_id: UUID, request: PromptGenerateRequest) -> PromptBundle:
        ...
        if request.plan_id:
            ...
            plan = db.query(Plan).filter(Plan.project_id == project_id).order_by(desc(Plan.version)).first()
        ...
        last_bundle = (
            ...
            .filter(PromptBundle.project_id == project_id)
            .order_by(desc(PromptBundle.version))
            .first()
        )
        ...
        # Generate context
        context_md = PromptService._generate_context(project, plan, requirements, request.include_code)
        ...
        for sequence, phase in enumerate(plan.phases, 1):
            prompt_content = PromptService._generate_phase_prompt(phase, requirements, project, request.include_code)
    ...
    @staticmethod
    def _generate_context(project: Project, plan: Plan, requirements: List[Requirement], include_code: bool) -> str:
    ...
    def _generate_phase_prompt(
        phase: PlanPhase, requirements: List[Requirement], project: Project, include_code: bool
    ) -> str:
    ...
    def _detect_tech_stack(requirements: List[Requirement]) -> List[str]:
        ...
        all_text = " ".join([f"{r.title} {r.description or ''}" for r in requirements]).lower()
        ...
        if not tech_stack:
            # Default stack
            tech_stack = ["Backend: Python/FastAPI", "Database: PostgreSQL", "Cache: Redis", "Container: Docker"]
    ...
    def create_bundle_zip(db: Session, bundle_id: UUID) -> bytes:
        ...
        bundle = db.query(PromptBundle).filter(PromptBundle.id == bundle_id).first()
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            # Add context file
            zip_file.writestr("context.md", bundle.context_md)
            ...
            metadata = {
                ...
                "prompts": [
                    {"phase_id": p.phase_id, "title": p.title, "sequence": p.sequence, "metadata": p.extra_metadata}
